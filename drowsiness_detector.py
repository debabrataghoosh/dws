...
# Performance tuning: process every Nth frame and downscale to target width
PROCESS_EVERY = 3
TARGET_WIDTH = 320
...
while True:
    ...
    # downscale to a fixed smaller size for faster processing
    try:
        frame = imutils.resize(frame, width = TARGET_WIDTH)
    except Exception:
        # fallback: skip this frame if resize fails
        continue

    # apply light removal / grayscale conversion once
    L, gray = lr.light_removing(frame)
    
    # Only run heavy dlib detection every PROCESS_EVERY frames
    frame_idx += 1
    rects = []
    ...
    if do_detect:
        try:
            ...
            # log and continue; avoid crashing the loop
            print(f"[warn] detector exception: {e}")
            rects = []
    ...
    for rect in rects:
        ...
        try:
            ...
            # If landmark processing fails for this face, skip it
            continue
        ...
        # print a compact status line occasionally to stdout for logging
        now = time.time()
        if now - last_log_time >= log_interval:
            try:
                print(f"EAR={both_ear:.2f} COUNTER={COUNTER} ALARMS={ALARM_COUNT}")
            except Exception:
                pass
            last_log_time = now
